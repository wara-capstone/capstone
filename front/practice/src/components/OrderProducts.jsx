...
async function optionGet() { //옵션 가져오기
    const response = await fetch(
        `${process.env.NODE_ENV === 'development' ? 'http:' : 'https:'}//${process.env.REACT_APP_API_URL}product/all/`+data.product.p_id,
        {
          method: "GET",
          headers: {    
...
async function optionEdit(){  //옵션 변경
    ...
    const payload = {
      ...
      try {
        const response = await fetch(
          `${process.env.NODE_ENV === 'development' ? 'http:' : 'https:'}//${CART_URL}cart/items/`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
