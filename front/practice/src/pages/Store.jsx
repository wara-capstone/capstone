...
export default function Store() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(`${process.env.NODE_ENV === 'development' ? 'http:' : 'https:'}//${process.env.REACT_APP_API_URL}store/read/id/` + id, {
        method: "GET",
        headers: {
  ...
  return (
    <div className="store">
      ...
      <Category
        allUrl={`${process.env.NODE_ENV === 'development' ? 'http:' : 'https:'}//${process.env.REACT_APP_API_URL}product/all/store/${id}`}
        categoryUrl={`${process.env.NODE_ENV === 'development' ? 'http:' : 'https:'}//${process.env.REACT_APP_API_URL}product/all/store/${id}/category?category=`}
      />
