...
const SellerStoreEdit = ({ store }) => {
  ...
  const fetchData = async (initMarkers) => {
    console.log(email);
    try {
      const response = await fetch(`${process.env.NODE_ENV === 'development' ? 'http://' : ''}${process.env.REACT_APP_API_URL}store/read/seller/${email}`, {
        method: "GET",
        headers: {
  ...
  const handleSubmit = (e) => {
    ...
    if (image) {
      ...
      fetch(`${process.env.NODE_ENV === 'development' ? 'http://' : ''}${process.env.REACT_APP_API_URL}store/update/id`, {
        method: "PUT",
        headers: {
          Authorization: `${token}`,
        },
  ...
  const handleDeleteStore = async (event) => {
    ...
    fetch(`${process.env.NODE_ENV === 'development' ? 'http://' : ''}${process.env.REACT_APP_API_URL}store/delete/id/${storeId}`, {
      method: "DELETE",
    })
      .then((response) => {
        if (response.ok) {
